...
def genSKU(pYear, pMonth, orionID, orionPass, connDetail, companyCode, sftpHost, sftpUser, sftpPass, sftpPath):
    ...
    try:
        ...
        sql = """SELECT DISTINCT ITEM_CODE,PLI_COMP_CODE COMPANY,ITEM_NAME,ITEM_IG_CODE ITEM_GROUP ,ITEM_ANLY_CODE_03 CATEGORY , AD_NAME BRAND,ITEM_UOM_CODE,ITEM_FLEX_01 GM,ITEM_FLEX_02 PKT 
        ...
        if numberOfrow > 0:
            try:
                ...
                mailJet.sendEmail("Error pushing SKU master file to SFTP server", "dev.team@tolaram.com","Hi Team, Please check. FTP can not connect "+connDetail + companyCode)
                mailJet.sendEmail("Error pushing SKU master file to SFTP server","moni.john@tolaram.com","Hi Team, Please check. FTP can not connect " +connDetail + companyCode)
                mailJet.sendEmail("Error pushing SKU master file to SFTP server","rao.topuri@tolaram.com","Hi Team, Please check. FTP can not connect " +connDetail + companyCode)
        ...
        logData(
	            'Error connecting database while running -skuMaster.py- for company - '+companyCode+' .\n')
