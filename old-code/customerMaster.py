...
def genCustomer(
    ...
    try:
        ...
        sql = (
            """SELECT DISTINCT CUST_CODE,CCO_COMP_CODE COMPANY,CUST_MAIN_ACNT_CODE,MAIN_ACNT_NAME,CUST_NAME,DECODE (CUST_FRZ_FLAG_NUM,'1','IN ACTIVE','ACTIVE' ) STATUS,
        ...
        if numberOfrow > 0:
            try:
                ...
                mailJet.sendEmail(
                    "Error pushing customer master file to SFTP server",
        ...
        logData(
            "Error connecting database while running -customerMaster.py- for company - "
            + companyCode
            + " .\n"
        )
